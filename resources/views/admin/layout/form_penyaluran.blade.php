...
@section('content')
                ...
                <div class="relative w-full">
                    ...
                    <div x-show="open" @click.prevent @click.away="open=false"
                        ...
                        <ul>
                            ...
                            {{-- <li>
                                <a href="#"
                                @click.prevent="selected = 'SemuaPangkalan'; isOpen = false; $refs.input.value = ''; $refs.input.dispatchEvent(new Event('input'))"
                                class="block px-4 py-2 text-gray-700 hover:bg-gray-100">Pilih Semua Pangkalan</a>
                            </li> --}}
    ...
    <script>
        ...
        btnSimpan.addEventListener('click', function () {
            ...
            const wilayahId = formData.get('wilayah');
            const pangkalanId = formData.get('pangkalan');

            // Ambil data dari tabel
            let rows = [];
            document.querySelectorAll('#tableBody tr').forEach(tr => {
                let row = {
                    pa_id: tr.children[0].textContent.trim(),
                    pangkalan_nama: tr.children[1].textContent.trim(),
                    pembayaran: tr.children[2].textContent.trim(), // Ambil nilai pembayaran dari kolom tabel
                    values: {}
                };

                // Ambil semua input number di baris
                tr.querySelectorAll('input[type=number]').forEach(input => {
                    const tanggal = input.getAttribute('data-date'); // dd-mm-yyyy
                    row.values[tanggal] = parseInt(input.value) || 0;
                });
        ...
        btnTampilkan.addEventListener('click', function () {
            ...
            console.log({ tanggalAwal, tanggalAkhir, kondisi, wilayahId, pangkalanId });
            ...
            if (pangkalanId === 'all') {
                // Ambil data pangkalan dari server sesuai wilayah
                fetch(`/perencanaan/pangkalantabel/${wilayahId}`)
                    .then(res => res.json())
                    .then(data => {
                        data.forEach(p => {
                            tableBody.innerHTML += `
                                <tr>
                                    ...
                                    <td class="px-4 py-2 border border-gray-300 truncate">${p.pklan_nama_pangkalan}</td>
                ...
                // === Jika pilih pangkalan tertentu ===
                tableBody.innerHTML = `
                    <tr>
                        <td class="px-4 py-2 border border-gray-300">${generateUniqueId()}</td>
                        <td class="px-4 py-2 border border-gray-300">${pangkalanId}</td>
                        <td class="px-4 py-2 border border-gray-300">CASHLESS</td>
                        ${generateDateCells(tanggalAwal, tanggalAkhir)}
                    </tr>
                `;
            ...
            tableFooter.innerHTML = `
                <tr>
                    ...
                    ${generateFooterCells(tanggalAwal, tanggalAkhir)}
                </tr>
            `;

            // Fungsi untuk membuat sel footer total
            function generateFooterCells(startDate, endDate) {
                let cells = '';
                for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                    const formattedDate = `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}`;
                    const fullDate = `${formattedDate}-${d.getFullYear()}`; // Format lengkap dd-mm-yyyy
                    cells += `
                        <td class="px-4 py-2 text-center font-bold border border-gray-300 bg-gray-100 footer-total" data-date="${fullDate}">
                            0
                        </td>
                    `;
                }
            ...
            // Fungsi untuk membuat header tanggal (dd-mm) dengan nilai data-date (dd-mm-yyyy)
            function generateDateHeaders(startDate, endDate) {
                let headers = '';
                for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                    const formattedDate = `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}`;
                    const fullDate = `${formattedDate}-${d.getFullYear()}`; // Format lengkap dd-mm-yyyy
                    headers += `<th class="px-4 py-2 text-center border-2 border-gray-400 truncate" data-date="${fullDate}">${formattedDate}</th>`;
                }
                return headers;
            }

            // Fungsi untuk membuat sel tanggal kosong dengan data-date (dd-mm-yyyy)
                            ...
                            <div class="flex justify-center items-center">
                                <input type="number"
                                    class="w-full h-full border bg-white text-center focus:outline-none 
                                        focus:ring-0 text-gray-700 [appearance:textfield] rounded-md p-1 mx-2 input-spinner"
                                    value="0"
                                    min="0"
            ...
            // Fungsi untuk menghitung total di footer tabel
            function updateTotal() {
                const totals = {};
                document.querySelectorAll('.input-spinner').forEach(function (input) {
                    const date = input.getAttribute('data-date'); // Ambil data-date (dd-mm-yyyy)
                ...
                // Update footer tabel
                const footerCells = document.querySelectorAll('#tableFooter .footer-total');
                footerCells.forEach((cell, index) => {
                    const date = cell.getAttribute('data-date'); // Ambil data-date dari footer
                    cell.textContent = totals[date] || 0; // Tampilkan total untuk tanggal tersebut
                });
            }

            // Tambahkan event listener untuk menghitung total saat nilai spinner berubah
            document.addEventListener('input', function (event) {
                if (event.target.classList.contains('input-spinner')) {
                    updateTotal();
                }
            });
        ...
        // Script untuk import file
        document.addEventListener('DOMContentLoaded', function () {
            const btnImport = document.getElementById('btnImport');
            const importFile = document.getElementById('importFile');

            // Event listener untuk tombol Import Data
            btnImport.addEventListener('click', function () {
                importFile.click(); // Trigger input file
            });

            // Event listener untuk file yang diunggah
            importFile.addEventListener('change', function () {
                const file = importFile.files[0];
                if (!file) {
                    alert('Harap pilih file untuk diimport.');
                    return;
                }
                ...
                fetch('/admin/perencanaan/import', {
                    method: 'POST',
                    headers: {
                        'X-CSRF-TOKEN': document.querySelector('input[name="_token"]').value,
                    },
                    ...
                    .then((data) => {
                        if (data.error) {
                            alert('Terjadi kesalahan saat mengimpor data: ' + JSON.stringify(data.error));
                        } else {
                            alert('Data berhasil diimport!');
                            populateTable(data); // Tampilkan data di tabel
                        }
                    })
                    .catch((error) => {
            ...
            // Fungsi untuk membuat sel tanggal kosong dengan data-date (dd-mm-yyyy)
            function generateDateCells(startDate, endDate, values = {}) {
                let cells = '';
                for (let d = new Date(startDate); d <= endDate; d.setDate(d.getDate() + 1)) {
                    const formattedDate = `${String(d.getDate()).padStart(2, '0')}-${String(d.getMonth() + 1).padStart(2, '0')}`;
                    const fullDate = `${formattedDate}-${d.getFullYear()}`; // Format lengkap dd-mm-yyyy
                    const value = values[fullDate] || 0; // Ambil nilai dari values jika ada, default 0
                    cells += `
                        <td class="border border-gray-300 text-center p-0">
                            <div class="flex justify-center items-center">
                                <input type="number"
            ...
            function populateTable(data) {
                ...
                const headerRow = `
                    <tr>
                        ...
                        <th class="px-4 py-2 text-left border-2 border-gray-400">Nama Pangkalan</th>
                        <th class="px-4 py-2 text-left border-2 border-gray-400">Pembayaran</th>
                        ${generateDateHeaders(data.tanggal_awal, data.tanggal_akhir)}
                    </tr>
                `;
                tableHeader.innerHTML = headerRow;
                ...
                data.rows.forEach((row) => {
                    tableBody.innerHTML += `
                        <tr>
                            <td class="px-4 py-2 border border-gray-300">${row.pa_id}</td>
                            <td class="px-4 py-2 border border-gray-300">${row.pangkalan_nama || 'Semua Pangkalan'}</td>
                            <td class="px-4 py-2 border border-gray-300">${row.pembayaran}</td>
                            ${generateDateCells(data.tanggal_awal, data.tanggal_akhir, row.values)}
                        </tr>
                    `;
                });
                ...
                tableFooter.innerHTML = `
                    <tr>
                        <td colspan="3" class="px-4 py-2 text-center font-bold border border-gray-300 bg-gray-100">Total</td>
                        ${generateFooterCells(data.tanggal_awal, data.tanggal_akhir)}
                    </tr>
                `;
            }
        });
